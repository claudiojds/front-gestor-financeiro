...
export default function Login() {
  ...
  const router = useRouter(); // Instancie o hook
      ...
      {isOpen && (
        <div className="absolute right-0 z-10 mt-1 w-50 origin-top-right rounded-md bg-[rgba(143,139,139,0.75)] shadow-lg">
          <div className="py-1">
            {!user && (
              <button
                className="w-full px-4 py-2 text-sm text-left text-[#181515] hover:bg-[rgba(143,139,139,0.75)] transition cursor-pointer"
            ...
            <button
              ...
              onClick={() => {
                setIsOpen(false);
                router.push("/index/configuracoes"); // Redireciona para a página de configurações
              }}
            >
              Configurações
            </button>
            {user && (
              <button
                className="w-full px-4 py-2 text-sm text-left text-[#181515] hover:bg-[rgba(143,139,139,0.75)] transition cursor-pointer"
                onClick={() => { setUser(null); localStorage.removeItem("user"); setIsOpen(false); }}
              >
                Sair
              </button>
            )}
